...
export async function GET(
  ...
  try {
    ...
    if (!session?.user?.id) {
      ...
      return NextResponse.redirect(new URL('/api/auth/signin?error=SessionRequired', request.url));
    }
    ...
    if (!landingPage) {
      ...
      return NextResponse.redirect(new URL('/?error=PageNotFound', request.url));
    }
    ...
    if (landingPage.isClaimed && landingPage.userId !== userId) {
      ...
      return NextResponse.redirect(new URL(`/p/${slug}?error=AlreadyClaimed`, request.url));
    }
    ...
    return NextResponse.redirect(new URL(`/p/${slug}?claimed=true`, request.url));
