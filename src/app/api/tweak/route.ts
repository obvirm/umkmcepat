...
export async function POST(request: Request) {
    try {
        ...
        const landingPage = await prisma.landingPage.findUnique({
            where: { slug },
            select: { id: true, userId: true, isClaimed: true, tweaksLeft: true, aiContent: true },
        });
        ...
        if (!landingPage.isClaimed || landingPage.userId !== userId) {
            return NextResponse.json({ message: 'Anda tidak punya izin untuk mengubah halaman ini' }, { status: 403 });
        }

        if (landingPage.tweaksLeft <= 0) {
            return NextResponse.json({ message: 'Jatah tweak AI sudah habis' }, { status: 400 });
        }

        const currentAiContent = (landingPage.aiContent || {}) as unknown as AiGeneratedContent;

        // 2. Call AI tweak function
        const newAiContent = await tweakLandingPageContent(currentAiContent, instruction);
